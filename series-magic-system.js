...
if (window.completeSeries) {
    console.log('✅ النظام محمل:', window.completeSeries);
    ...
    testDiv.innerHTML = `
        ...
        ">
            🎬 نظام المسلسلات الذكي يعمل!
            <br><small>67 مسلسل جاهز</small>
        </div>
    `;
    document.body.appendChild(testDiv);
    
    console.log('✅ تم إضافة عنصر الاختبار');
} else {
    console.log('❌ النظام غير محمل');
    
    // حيلة لتحميل النظام يدوياً
    setTimeout(() => {
        if (!window.completeSeries) {
            console.log('🔄 محاولة تحميل يدوي...');
            const script = document.createElement('script');
            script.src = 'js/series-magic-system.js';
            document.body.appendChild(script);
        }
    }, 2000);
}

// تحقق من عناصر الصفحة
console.log('📊 حالة الصفحة:', {
    body: document.body ? '✅ موجود' : '❌ مفقود',
    scripts: document.querySelectorAll('script').length + ' scripts',
    hasChatbot: !!document.querySelector('.chatbot-widget'),
    hasTrending: !!document.querySelector('.trending-section')
});